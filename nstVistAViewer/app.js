...
var application = {
	...
	onFragment: {
		...
		'ddFieldDetail.html' : function(messageObj) {
			...
			'infoDetail.html' : function(messageObj) {
			  // has to be here. Otherwise it won't load it.
					
			},
			
			'patchRoutineList.html' : function(messageObj) {
			  // has to be here. Otherwise it won't load it.
					
			},
						
			'ddData.html' : function(messageObj) {
			  // has to be here. Otherwise it won't load it.
	...
	onMessage: {
		...
		'displayRPCDetails' : function (messageObj) {			
				...
				application.session.rpc = messageObj.message.params;
	...
	onStartup :  function() {
		...
		// Selection panes
		params = {name: 'patchSelection.html', targetId: 'patch_SelectionPane'};
		EWD.getFragment(params, application.onFragment[params.name]); 
		
		params = {name: 'rpcSelection.html', targetId: 'rpc_SelectionPane'};
		EWD.getFragment(params, application.onFragment[params.name]); 
		
		params ={name: 'ddSelection.html', targetId: 'dd_SelectionPane'};
		EWD.getFragment(params, application.onFragment[params.name]); 
				
		params ={name: 'selectionPane.html', targetId: 'other_SelectionPane'};
		EWD.getFragment(params, application.onFragment[params.name]); 

		
		// Details 
		params ={name: 'patchSummary.html', targetId: 'patch_SummaryPane'}; 
		EWD.getFragment(params, application.onFragment[params.name]); 
		
		params ={name: 'ddFieldDetail.html',targetId:  'ddFieldDetail_Container'}; 
		EWD.getFragment(params, application.onFragment[params.name]); 
		
		params ={name: 'ddData.html', targetId: 'ddData_Container'};		
		EWD.getFragment(params, application.onFragment[params.name]); 
		
		params ={name: 'patchRoutineList.html', targetId: 'patchRoutineList_Container'};		
		EWD.getFragment(params, application.onFragment[params.name]); 
		
		params ={name: 'routineDetail.html', targetId: 'routine_Container'}; 	
		EWD.getFragment(params, application.onFragment[params.name]); 
		
		params ={name: 'rpcDetail.html', targetId: 'rpc_Container'};
		EWD.getFragment(params, application.onFragment[params.name]); 
		
		params ={name: 'infoDetail.html', targetId: 'info_Container'}; 		
		EWD.getFragment(params, application.onFragment[params.name]); 
		
		params ={name: 'rpcTesterForm.html', targetId: 'RPCTesterForm'}; 
		EWD.getFragment(params, application.onFragment[params.name]); 
...
$(document).ready(function() {
  ...
  EWD.on('ewd-registered', function() {
	...
	// Attach application messages
	for (var message in application.onMessage) {
		EWD.on(message, application.onMessage[message]);
	};
